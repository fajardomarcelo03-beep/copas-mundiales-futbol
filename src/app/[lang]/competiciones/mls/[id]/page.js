...
export default function DetalleMLSPage() {
  ...
  const objetoNoticia = noticiasMLS.find(n => n.id === id);

  useEffect(() => {
    if (noticiasMLS && id) {
      const otras = noticiasMLS.filter(n => n.id !== id);
  ...
  if (!objetoNoticia) {
    return (
      <div style={{ ...containerStyle, justifyContent: 'center' }}>
        ...
        <Link href={`/${idioma}/competiciones/mls`} style={btnVolverStyle}>
          {idioma === 'es' ? '← Volver a MLS' : '← Back to MLS'}
        </Link>
      </div>
    );
  }
    ...
    <div style={containerStyle} className="vista-detalle-noticia-raiz">
      <div style={cardStyle} className="cuerpo-articulo-card">
        <div style={headerNoticiaStyle} className="header-noticia">
          <span style={tagStyle}>🔴 MLS 2026</span>
          ...
          <>
            ...
            <div style={gridSugerenciasStyle} className="grid-sugerencias">
              {sugerenciasAleatorias.map((sug) => (
                <Link key={sug.id} href={`/${idioma}/competiciones/mls/${sug.id}`} style={enlaceSugerenciaStyle}>
