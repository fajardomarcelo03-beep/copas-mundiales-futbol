...
export default function DetalleLigaProPage() {
  ...
  const objetoNoticia = noticiasLigaPro.find(n => n.id === id);

  useEffect(() => {
    if (noticiasLigaPro && id) {
      const otras = noticiasLigaPro.filter(n => n.id !== id);
  ...
  if (!objetoNoticia) {
    return (
      <div style={{ ...containerStyle, justifyContent: 'center' }}>
        ...
        <Link href={`/${idioma}/competiciones/ligapro`} style={btnVolverStyle}>
          {idioma === 'es' ? '← Volver a LigaPro' : '← Back to LigaPro'}
        </Link>
      </div>
    );
  }
    ...
    <div style={containerStyle} className="vista-detalle-noticia-raiz">
      <div style={cardStyle} className="cuerpo-articulo-card">
        <div style={headerNoticiaStyle} className="header-noticia">
          <span style={tagStyle}>🔴 LIGAPRO 2026</span>
