...
export default function DetallePremierPage() {
  ...
  const objetoNoticia = noticiasPremier.find(n => n.id === id);

  useEffect(() => {
    if (noticiasPremier && id) {
      const otras = noticiasPremier.filter(n => n.id !== id);
  ...
  if (!objetoNoticia) {
    return (
      <div style={{ ...containerStyle, justifyContent: 'center' }}>
        ...
        <Link href={`/${idioma}/competiciones/premier-league`} style={btnVolverStyle}>
          {idioma === 'es' ? '← Volver a Premier League' : '← Back to Premier League'}
        </Link>
      </div>
    );
  }
    ...
    <div style={containerStyle} className="vista-detalle-noticia-raiz">
      <div style={cardStyle} className="cuerpo-articulo-card">
        <div style={headerNoticiaStyle} className="header-noticia">
          <span style={tagStyle}>🔴 PREMIER LEAGUE 2026</span>
