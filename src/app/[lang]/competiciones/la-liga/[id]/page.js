...
export default function DetalleLaLigaPage() {
  ...
  const objetoNoticia = noticiasLaLiga.find(n => n.id === id);

  useEffect(() => {
    if (noticiasLaLiga && id) {
      const otras = noticiasLaLiga.filter(n => n.id !== id);
    ...
    <div style={containerStyle} className="vista-detalle-noticia-raiz">
      <div style={cardStyle} className="cuerpo-articulo-card">
        <div style={headerNoticiaStyle} className="header-noticia">
          <span style={tagStyle}>🔴 LALIGA 2026</span>
